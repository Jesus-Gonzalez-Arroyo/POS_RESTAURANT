...
export const addCategory = async (category: Category) => {
    const { name, description, color, icon, isActive, createdAt, updatedAt } = category;
    const res = await pool.query(
        'INSERT INTO categories (name, description, color, icon, is_active, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
        [name, description, color, icon, isActive, createdAt, updatedAt]
    );
    return res.rows[0];
}
...
export const updateCategory = async (id: number, category: Category) => {
    ...
    const res = await pool.query(
        ...
        [name, description, color, icon, isActive, createdAt, updatedAt, id]
    );
    return res.rows[0];
}
